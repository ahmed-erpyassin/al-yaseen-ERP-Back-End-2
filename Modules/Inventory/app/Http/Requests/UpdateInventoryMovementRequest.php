...
class UpdateInventoryMovementRequest extends FormRequest
{
    ...
    /**
     * ✅ Get the validation rules for updating inventory movement.
     */
    public function rules(): array
    {
        $movementId = $this->route('id') ?? $this->route('inventory_movement');
        ...
        return [
            ...
            'movement_number' => [
                ...
                'string',
                'max:255',
                Rule::unique('inventory_movements', 'movement_number')->ignore($movementId)
            ],
            ...
            // ✅ Movement Data (Details) - Optional for updates
            'movement_data' => 'nullable|array',
            'movement_data.*.id' => 'nullable|integer', // For updating existing items
            ...
            'movement_data.*._delete' => 'nullable|boolean', // For marking items for deletion
        ];
    }
