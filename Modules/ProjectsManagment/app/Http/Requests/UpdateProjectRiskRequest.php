...
class UpdateProjectRiskRequest extends FormRequest
{
    ...
    {
        $user = $this->user();
        $projectRiskId = $this->route('id') ?? $this->route('project_risk');
        
        return [
            // Project selection (optional for updates)
            'project_id' => [
                'sometimes',
                'required',
                'integer',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Custom validation: Check for duplicate risk titles within the same project (excluding current record)
