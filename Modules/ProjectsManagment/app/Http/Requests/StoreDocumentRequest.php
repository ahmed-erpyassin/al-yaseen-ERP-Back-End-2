...
class StoreDocumentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'file' => 'required|file|max:20480|mimes:pdf,doc,docx,xls,xlsx,ppt,pptx,txt,jpg,jpeg,png,gif,zip,rar',
    ...
    protected function prepareForValidation(): void
    {
        ...
        if (!$this->has('upload_date') || empty($this->upload_date)) {
            $this->merge(['upload_date' => now()->toDateString()]);
        }

        if (!$this->has('version') || empty($this->version)) {
            $this->merge(['version' => '1.0']);
        }

        if (!$this->has('document_category') || empty($this->document_category)) {
            $this->merge(['document_category' => 'other']);
        }
    }
